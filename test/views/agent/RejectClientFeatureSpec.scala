...
class RejectClientFeatureSpec extends AnyFeatureSpec
  ...
  Feature("The user can view the reject client page") {
    ...
    Scenario("user has visited the page") {
      ...
      Then("The title should match - Are you sure you want to reject the request from this client? - GOV.UK - service.name - site.govuk")
      assert(document.title() === "agent.reject-client.title - GOV.UK - service.name - site.govuk")
