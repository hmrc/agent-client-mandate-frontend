...
class ReviewMandateControllerSpec extends PlaySpec with MockitoSugar with BeforeAndAfterEach with MockControllerSetup with GuiceOneServerPerSuite with TestApplicationBuilder {
    ...
    "return review mandate view for AUTHORISED client" when {
      ...
      "client requests(GET) for review mandate view, and mandate has been cached on search mandate submit" in new Setup {
        ...
        viewWithAuthorisedClient(reviewMandateController)(Some(returnData)) { result =>
          ...
          document.title() must be("client.review-agent.title - service.name - GOV.UK")
