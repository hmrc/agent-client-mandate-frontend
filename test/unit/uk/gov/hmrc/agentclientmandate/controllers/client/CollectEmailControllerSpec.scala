...
class CollectEmailControllerSpec
  extends PlaySpec
    with MockitoSugar
    ...
    "return search mandate view for AUTHORISED client" when {
      ...
      "client requests(GET) for collect email view and the data hasn't been cached" in new Setup {
        viewWithAuthorisedClient() { result =>
          ...
          document.select("label.govuk-label").text() must include("client.collect-email.header")
          document.select("h2").text() must include("ated.screen-reader.section client.collect-email.preheader")
          document.getElementsByClass("govuk-label").text() must be("client.collect-email.header")
