...
class HasClientRegisteredBeforeControllerSpec extends PlaySpec with BeforeAndAfterEach with MockitoSugar with MockControllerSetup with GuiceOneServerPerSuite with TestApplicationBuilder {
    ...
    "redirect to 'has client registered page'" when {
      "agent requests(GET) for 'has client registered page', with service = ATED" in new Setup {
        viewWithAuthorisedAgent(service, ControllerPageIdConstants.paySAQuestionControllerId, Some(PrevRegistered(Some(true)))) { result =>
          ...
          document.title() must include("agent.client-prev-registered.title - service.name - GOV.UK")
        }
      }
