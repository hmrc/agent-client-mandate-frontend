...
class EditMandateDetailsControllerSpec extends PlaySpec with MockitoSugar with BeforeAndAfterEach with MockControllerSetup with GuiceOneServerPerSuite with TestApplicationBuilder {
    ...
    "return 'edit mandate' view" when {
      "clientParty exist for the mandate fetched" in new Setup {
        viewWithAuthorisedAgent(Some(mandate)) { result =>
          ...
          document.title() must be("agent.edit-mandate-details.title - service.name - GOV.UK")
