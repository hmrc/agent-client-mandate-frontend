...
class OverseasClientQuestionControllerSpec extends PlaySpec with MockitoSugar with BeforeAndAfterEach with MockControllerSetup with GuiceOneServerPerSuite with TestApplicationBuilder {
      ...
      "invalid form is submitted" in new Setup {
        ...
        submitWithAuthorisedAgent(fakeRequest) { result =>
          ...
          document.getElementsByClass("govuk-error-message")
            .text() mustBe "govukErrorMessage.visuallyHiddenText: agent.overseas-client-question.error.isOverseas"
        }
      }
    }
  }
