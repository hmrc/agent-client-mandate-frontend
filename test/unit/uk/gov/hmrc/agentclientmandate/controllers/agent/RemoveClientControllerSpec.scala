...
class RemoveClientControllerSpec extends PlaySpec with MockitoSugar with BeforeAndAfterEach with MockControllerSetup with GuiceOneServerPerSuite with TestApplicationBuilder {
  ...
  val mandate: Mandate = Mandate(id = "1", createdBy = User("credId", "agentName", Some("agentCode")), None, None,
    ...
    currentStatus = MandateStatus(Status.New, Instant.now(), "credId"), statusHistory = Nil,
  ...
  "return 'remove client question' view for AUTHORISED agent" when {
    ...
    "agent requests(GET) for 'remove client question' view" in new Setup {
      ...
      viewWithAuthorisedAgent { result =>
        ...
        document.title() must be("agent.remove-client.header - service.name - GOV.UK")
        document.getElementsByTag("header").text() must include("ated.screen-reader.section agent.edit-mandate-details.pre-header")
        document.getElementsByTag("header").text() must include("agent.remove-client.header")
  ...
  "returns BAD_REQUEST" when {
    "invalid form is submitted" in new Setup {
      ...
      submitWithAuthorisedAgent(fakeRequest) { result =>
        ...
        val document = Jsoup.parse(contentAsString(result))
        document.getElementsByClass("govuk-error-summary__body").text() mustBe "yes-no.error.mandatory.removeClient"
        document.getElementsByClass("govuk-error-message").text() mustBe "govukErrorMessage.visuallyHiddenText: yes-no.error.mandatory.removeClient"
      }
    }
  }
  ...
  "submitting form " when {
    ...
    "submitted with true throws exception" in new Setup {
      when(mockAgentClientMandateService.removeClient(ArgumentMatchers.any(),
        ArgumentMatchers.any())(using ArgumentMatchers.any(), ArgumentMatchers.any())) thenReturn Future.successful(false)
      val userId = s"user-${UUID.randomUUID}"
  ...
  "return 'client remove confirmation' view for AUTHORISED agent" when {
    ...
    "agent requests(GET) for 'client remove confirmation' view" in new Setup {
      ...
      showConfirmationWithAuthorisedAgent { result =>
        status(result) must be(OK)
        val document = Jsoup.parse(contentAsString(result))
        document.title() must be("agent.remove-client-confirmation.title - service.name - GOV.UK")
      }
    }
  }
