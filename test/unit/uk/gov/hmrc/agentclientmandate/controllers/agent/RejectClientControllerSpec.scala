...
class RejectClientControllerSpec extends PlaySpec with MockitoSugar with BeforeAndAfterEach with MockControllerSetup with GuiceOneServerPerSuite with TestApplicationBuilder {
  ...
  val mandate: Mandate = Mandate(id = "1", createdBy = User("credId", "agentName", Some("agentCode")), None, None,
    ...
    clientParty = Some(Party("JARN123456", "ACME Limited", PartyType.Organisation,
      ContactDetails("client@client.com", None))), currentStatus = MandateStatus(Status.New, Instant.now(), "credId"),
    ...
    "return 'reject client question' view for AUTHORISED agent" when {
      ...
      "agent requests(GET) for 'reject client question' view" in new Setup {
        ...
        viewWithAuthorisedAgent { result =>
          ...
          document.title() must be("agent.reject-client.title - service.name - GOV.UK")
