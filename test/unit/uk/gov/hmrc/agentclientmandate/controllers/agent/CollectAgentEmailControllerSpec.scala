...
class CollectAgentEmailControllerSpec extends PlaySpec with MockitoSugar with BeforeAndAfterEach with MockControllerSetup with GuiceOneServerPerSuite with TestApplicationBuilder {
  ...
  val injectedViewInstanceAgentEnterEmail: agentEnterEmail = app.injector.instanceOf[views.html.agent.agentEnterEmail]
  ...
  class Setup {
    ...
    def editEmailAuthorisedAgent(cachedData: Option[AgentEmail] = None, redirectUrl: Option[String]=None)(test: Future[Result] => Any): Unit = {
      val userId = s"user-${UUID.randomUUID}"
    ...
    def retrieveAgentEmailFromSessionAuthorisedAgent(cachedData:  Option[AgentEmail] = None)(test: Future[Result] => Any): Unit = {
      val userId = s"user-${UUID.randomUUID}"
    ...
    "return 'what is your email address' for AUTHORISED agent" when {
      ...
      "agent requests(GET) for 'what is your email address' view and the data hasn't been cached" in new Setup {
        viewEmailAuthorisedAgent(None, Some(RedirectUrl("/api/anywhere"))) { result =>
          ...
          val document = Jsoup.parse(contentAsString(result))
          document.title() must be("agent.enter-email.title - service.name - GOV.UK")
          document.select("h1 > label.govuk-label--xl").text() must include("agent.enter-email.header")
          document.select("h2.govuk-caption-l > span").text() must include("ated.screen-reader.section agent.add-a-client.sub-header")
          document.getElementById("email-hint").text() must include(s"agent.enter-email.info.text")
          document.getElementsByClass("govuk-label").text() must be("agent.enter-email.header")
      ...
      "agents try to edit their email address redirecting to 'what is your email address' view and the data hasn't been cached" in new Setup {
        editEmailAuthorisedAgent(None, Some("/api/anywhere")) { result =>
          ...
          document.title() must be("agent.enter-email.title - service.name - GOV.UK")
          document.select("h1 > label.govuk-label--xl").text() must include("agent.enter-email.header")
          document.select("h2 > span").text() must include("ated.screen-reader.section agent.add-a-client.sub-header")
          document.getElementById("email-hint").text() must include(s"agent.enter-email.info.text")
          document.getElementsByClass("govuk-label").text() must be("agent.enter-email.header")
          document.getElementById("email").`val`() must be("")
          document.getElementById("submit").text() must be("continue-button")
      ...
      "agent requests(GET) for 'what is your email address' view pre-populated and the data has been cached" in new Setup {
        ...
        val clientMandatDisplay: ClientMandateDisplayDetails = ClientMandateDisplayDetails("name", "mandateId", "agent@mail.com")
