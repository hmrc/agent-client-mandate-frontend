...
class UpdateAddressDetailsControllerSpec extends PlaySpec with MockitoSugar with BeforeAndAfterEach with MockControllerSetup with GuiceOneServerPerSuite with TestApplicationBuilder {
  ...
  val updateRegDetails: Option[UpdateRegistrationDetailsRequest] = Some(UpdateRegistrationDetailsRequest(acknowledgementReference, isAnIndividual = false, None,
