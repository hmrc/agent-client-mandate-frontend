...
import uk.gov.hmrc.agentclientmandate.viewModelsAndForms.PrevUniqueAuthNumForm._
...
@Singleton
class PreviousUniqueAuthorisationNumberController @Inject()(
  ...
  def view(service: String, callingPage: String): Action[AnyContent] = Action.async { implicit request =>
    withAgentRefNumber(Some(service)) { _ =>
      for {
        prevUniqueAuthNum <- dataCacheService.fetchAndGetFormData[PrevUniqueAuthNum](prevUniqueAuthNumFormId)
    ...
    implicit request =>
      withAgentRefNumber(Some(service)) { _ =>
        prevUniqueAuthNumForm.bindFromRequest().fold(
          ...
          data => {
            dataCacheService.cacheFormData[PrevUniqueAuthNum](prevUniqueAuthNumFormId, data)
            val result = if (data.authNum.getOrElse(false)) {
              Redirect(routes.PreviousMandateRefController.view(callingPage))
            } else {
              Redirect(routes.InformHmrcController.view(callingPage))
            }
            Future.successful(result)
          }
        )
