...
class AgentClientMandateConnector @Inject()(val servicesConfig: ServicesConfig,
  ...
  def fetchMandate(mandateId: String)(using hc: HeaderCarrier, ec: ExecutionContext): Future[HttpResponse] = {
    val getUrl = s"$serviceUrl/$mandateUri/$mandateId"
    http.get(url"$getUrl").execute[HttpResponse]
  }

  def approveMandate(mandate: Mandate, authRetrievals: ClientAuthRetrievals)(using hc: HeaderCarrier, ec: ExecutionContext): Future[HttpResponse] = {
  ...
  def rejectClient(mandateId: String, agentCode: String)(using hc: HeaderCarrier, ec: ExecutionContext): Future[HttpResponse] = {
  ...
  def fetchAgentDetails()(using hc: HeaderCarrier, ec: ExecutionContext): Future[AgentDetails] = {
    val authLink = s"/agent"
    val getUrl = s"$serviceUrl$authLink/$mandateUri/agentDetails"
    http.get(url"$getUrl").execute[AgentDetails]
  }

  def activateMandate(mandateId: String, agentCode: String)(using hc: HeaderCarrier, ec: ExecutionContext): Future[HttpResponse] = {
    val authLink = s"/agent/$agentCode"
  ...
  def editMandate(mandate: Mandate, agentAuthRetrievals: AgentAuthRetrievals)(using hc: HeaderCarrier, ec: ExecutionContext): Future[HttpResponse] = {
  ...
  def fetchMandateByClient(clientId: String, service: String)(using hc: HeaderCarrier, ec: ExecutionContext): Future[HttpResponse] = {
  ...
  def updateAgentMissingEmail(emailAddress: Option[String], agentAuthRetrievals: AgentAuthRetrievals, service: String)
                             (using hc: HeaderCarrier, ec: ExecutionContext): Option[Future[HttpResponse]]= emailAddress map { email =>
  ...
  def updateAgentCredId(mandateAuthRetrievals: AgentAuthRetrievals)(using hc: HeaderCarrier, ec: ExecutionContext): Future[HttpResponse] = {
  ...
  // $COVERAGE-OFF$
  def testOnlyCreateMandate(mandate: Mandate)(using hc: HeaderCarrier, ec: ExecutionContext): Future[HttpResponse] = {
    val jsonData = Json.toJson(mandate)
  ...
  def testOnlyDeleteMandate(mandateId: String)(using hc: HeaderCarrier, ec: ExecutionContext): Future[HttpResponse] = {
